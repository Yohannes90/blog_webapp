...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('We have sent you an email with instructions to reset your password.', 'info')
        return redirect(url_for('users.login'))
...
def reset_token(token):
    ...
    if user is None:
        flash('The token you entered is expired or invalid', 'warning')
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password have been successfully updated! You can now login.', 'success')
        return redirect(url_for('users.login'))
